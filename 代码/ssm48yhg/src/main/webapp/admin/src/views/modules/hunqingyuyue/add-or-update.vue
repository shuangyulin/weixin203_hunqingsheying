<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="婚庆标题" prop="hunqingbiaoti">
          <el-input v-model="ruleForm.hunqingbiaoti" 
              placeholder="婚庆标题" clearable  :readonly="ro.hunqingbiaoti"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="婚庆标题" prop="hunqingbiaoti">
              <el-input v-model="ruleForm.hunqingbiaoti" 
                placeholder="婚庆标题" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.hunqingfengmian" label="婚庆封面" prop="hunqingfengmian">
          <file-upload
          tip="点击上传婚庆封面"
          action="file/upload"
          :limit="3"
          :multiple="true"
          :fileUrls="ruleForm.hunqingfengmian?ruleForm.hunqingfengmian:''"
          @change="hunqingfengmianUploadChange"
          ></file-upload>
        </el-form-item>
        <div v-else>
          <el-form-item v-if="ruleForm.hunqingfengmian" label="婚庆封面" prop="hunqingfengmian">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.hunqingfengmian.split(',')" :src="$base.url+item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="费用" prop="feiyong">
          <el-input v-model="ruleForm.feiyong" 
              placeholder="费用" clearable  :readonly="ro.feiyong"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="费用" prop="feiyong">
              <el-input v-model="ruleForm.feiyong" 
...
export default {
  ...
  methods: {
	...
	if(this.ruleForm.hunqingfengmian!=null) {
		this.ruleForm.hunqingfengmian = this.ruleForm.hunqingfengmian.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `hunqingyuyue/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.hunqingyuyueCrossAddOrUpdateFlag = false;
						 this.parent.search();
    ...
    hunqingfengmianUploadChange(fileUrls) {
	this.ruleForm.hunqingfengmian = fileUrls;
	this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
